...
#define DT_DRV_COMPAT renesas_rcar_gen3_gpio
...
static inline uint32_t gpio_rcar_read(const struct gpio_rcar_cfg *config, uint32_t offs)
{
	return sys_read32(config->reg_addr + offs);
}

static inline void gpio_rcar_write(const struct gpio_rcar_cfg *config, uint32_t offs,
				   uint32_t value)
{
	sys_write32(value, config->reg_addr + offs);
}

static void gpio_rcar_modify_bit(const struct gpio_rcar_cfg *config, uint32_t offs,
				 int bit, bool value)
{
...
static void gpio_rcar_config_general_input_output_mode(const struct gpio_rcar_cfg *config,
						       uint32_t gpio,
						       bool output)
{
				   ...
				   enum gpio_int_trig trig)
{
	...
	gpio_rcar_modify_bit(gpio_config, POSNEG, pin, (trig == GPIO_INT_TRIG_LOW));
