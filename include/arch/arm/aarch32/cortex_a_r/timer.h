...
static ALWAYS_INLINE void arm_arch_timer_set_compare(uint64_t val)
{
	...
	ctrl = sys_read32(ARM_ARCH_TIMER_BASE + TIMER_CTRL);
	ctrl &= ~(TIMER_COMP_ENABLE | TIMER_IRQ_ENABLE);
	sys_write32(ctrl, ARM_ARCH_TIMER_BASE + TIMER_CTRL);

	sys_write32(lower, ARM_ARCH_TIMER_BASE + TIMER_CMP_LOWER);
	sys_write32(upper, ARM_ARCH_TIMER_BASE + TIMER_CMP_UPPER);
...
static ALWAYS_INLINE void arm_arch_timer_set_irq_mask(bool mask)
{
	...
	if (mask) {
		...
		ctrl |= TIMER_IRQ_ENABLE;
		sys_write32(1, ARM_ARCH_TIMER_BASE + TIMER_ISR);
	}
	sys_write32(ctrl, ARM_ARCH_TIMER_BASE + TIMER_CTRL);
}
...
static ALWAYS_INLINE uint64_t arm_arch_timer_count(void)
{
	 ...
	 */
	upper = sys_read32(ARM_ARCH_TIMER_BASE + TIMER_CNT_UPPER);
	do {
		upper_saved = upper;
		lower = sys_read32(ARM_ARCH_TIMER_BASE + TIMER_CNT_LOWER);
		upper = sys_read32(ARM_ARCH_TIMER_BASE + TIMER_CNT_UPPER);
