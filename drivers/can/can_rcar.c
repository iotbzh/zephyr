...
static void can_rcar_tx_done(const struct device *dev)
{
	...
	tx_cb =	&data->tx_cb[data->tx_tail % RCAR_CAN_FIFO_DEPTH];
	data->tx_tail++;
	if (tx_cb->cb != NULL) {
...
static void can_rcar_state_change(const struct device *dev, uint32_t newstate)
{
	...
	LOG_INF("Can state change new: %u old:%u\n", newstate, data->state);
...
static void can_rcar_int_thread(const struct device *dev)
{
	const struct can_rcar_cfg *config = DEV_CAN_CFG(dev);
	struct can_rcar_data *data = DEV_CAN_DATA(dev);
	uint8_t unsent;
	...
	while (1) {
		...
		/* while there is pending messages to read */
		...
		/* lock inst_mutex to make sure tx_head doesn't grow */
		k_mutex_lock(&data->inst_mutex, K_FOREVER);
		while (1) {
			unsent = sys_read8(config->reg_addr + RCAR_CAN_TFCR);
			unsent = (unsent & RCAR_CAN_TFCR_TFUST) >>
				RCAR_CAN_TFCR_TFUST_SHIFT;
			if (data->tx_head - data->tx_tail <= unsent) {
				break;
			}
		  ...
		  void *callback_arg)
{
	...
	k_mutex_lock(&data->inst_mutex, K_FOREVER);
	tx_cb = &data->tx_cb[data->tx_head % RCAR_CAN_FIFO_DEPTH];
	tx_cb->cb = callback;
...
static int can_rcar_init(const struct device *dev)
{
	...
	k_thread_create(&data->int_thread, data->int_thread_stack,
			...
			(k_thread_entry_t)can_rcar_int_thread, (void *)dev,
