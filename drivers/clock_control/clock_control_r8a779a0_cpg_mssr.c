...
#define DT_DRV_COMPAT renesas_r8a779a0_cpg_mssr
...
struct r8a779a0_cpg_mssr_config {
...
static int r8a779a0_cpg_mssr_start(const struct device *dev,
				  clock_control_subsys_t sys)
{
	return r8a779a0_cpg_mssr_start_stop(dev, sys, true);
}

static int r8a779a0_cpg_mssr_stop(const struct device *dev,
				 clock_control_subsys_t sys)
{
	return r8a779a0_cpg_mssr_start_stop(dev, sys, false);
}

static int r8a779a0_cpg_get_rate(const struct device *dev,
				clock_control_subsys_t sys,
				uint32_t *rate)
{
	struct rcar_cpg_clk *clk = (struct rcar_cpg_clk *)sys;
...
static int r8a779a0_cpg_mssr_init(const struct device *dev)
{
	ARG_UNUSED(dev);
	return 0;
}

static const struct clock_control_driver_api r8a779a0_cpg_mssr_api = {
	.on = r8a779a0_cpg_mssr_start,
	.off = r8a779a0_cpg_mssr_stop,
...
#define R8A779a0_MSSR_INIT(inst)							  \
	...
	DEVICE_DT_INST_DEFINE(inst,						  \
			      &r8a779a0_cpg_mssr_init,				  \
			      NULL,						  \
			      NULL, &r8a779a0_cpg_mssr##inst##_config,		  \
			      PRE_KERNEL_1,					  \
			      CONFIG_CLOCK_CONTROL_INIT_PRIORITY,		  \
			      &r8a779a0_cpg_mssr_api);

DT_INST_FOREACH_STATUS_OKAY(R8A779a0_MSSR_INIT)
